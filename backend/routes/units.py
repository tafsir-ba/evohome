...
async def get_unit(unit_id: str, user=Depends(get_current_user)):
    ...
    if user.get("role") == "agent":
        (await resolve_agent_access_scope(user)).ensure_project_access(unit["project_id"])
...
async def attach_client_to_unit(unit_id: str, client_id: str, user=Depends(get_current_agent)):
    ...
    if not unit:
        raise HTTPException(status_code=404, detail="Unit not found")
    scope = await resolve_agent_access_scope(user)
    scope.ensure_project_access(unit["project_id"])

    client = await client_service.get_client(client_id)
    if not client:
        raise HTTPException(status_code=404, detail="Client not found")
    if client.get("agent_id") != scope.workspace_owner_id:
